...
class Process {
   ...
   * @param \Closure $processor Processing function (Default:
   *   {@link Utils::fullProcess}.
   * @param \Closure $scorer Scoring function (Default:
   *   {@link Fuzz::weightedRatio}).
  ...
  public function extract($query, $choices = [], $processor = NULL, $scorer = NULL, $limit = 5) {
    ...
    return $scored
      ->multiSort($scored->map($this->getScore)
        ->toArray(), SORT_DESC, SORT_NUMERIC)
      ->slice(0, $limit);
  }
  ...
  /**
   * Returns a Collection that has been filtered for duplicates using fuzzy
   * matching.
   *
   * @param array|\Traversable $containsDupes List containing duplicate
   *   strings.
      ...
      if ($filtered->count() === 1) {
        ...
        if (phpversion() > 5.5) {
          ...
          $filtered = $filtered->multiSort(
            ...
            $filtered->map($this->getScore)
              ->toArray(), SORT_DESC, SORT_NUMERIC,
            $filtered->map($this->getString)
              ->toArray(), SORT_ASC, SORT_STRING | SORT_FLAG_CASE
          );
        }
        else{
          $filtered = $filtered->multiSort(
